export const employees = [
  {
    id: "EMP001",
    name: "Sarah Johnson",
    email: "sarah.johnson@nexushr.com",
    phone: "+1 (555) 234-5678",
    department: "Engineering",
    role: "Senior Developer",
    designation: "Senior Software Engineer",
    status: "Active",
    joinDate: "2021-03-15",
    salary: 95000,
    grossSalary: 9500,
    deductions: 1140,
    netPay: 8360,
    avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "New York, NY",
    manager: "Robert Chen",
    employmentType: "Full-time",
    gender: "Female",
    dob: "1990-06-12",
    address: "123 Broadway Ave, New York, NY 10001",
    emergencyContact: "Michael Johnson — +1 (555) 345-6789",
    performance: 92,
  },
  {
    id: "EMP002",
    name: "Marcus Williams",
    email: "marcus.williams@nexushr.com",
    phone: "+1 (555) 345-6789",
    department: "Marketing",
    role: "Marketing Manager",
    designation: "Marketing Manager",
    status: "Active",
    joinDate: "2020-07-22",
    salary: 85000,
    grossSalary: 8500,
    deductions: 1020,
    netPay: 7480,
    avatar: "https://images.unsplash.com/photo-1651684215020-f7a5b6610f23?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Chicago, IL",
    manager: "Lisa Park",
    employmentType: "Full-time",
    gender: "Male",
    dob: "1988-11-23",
    address: "456 Michigan Ave, Chicago, IL 60601",
    emergencyContact: "Diana Williams — +1 (555) 456-7890",
    performance: 88,
  },
  {
    id: "EMP003",
    name: "Yuki Tanaka",
    email: "yuki.tanaka@nexushr.com",
    phone: "+1 (555) 456-7890",
    department: "Design",
    role: "UX Designer",
    designation: "Lead UX Designer",
    status: "Active",
    joinDate: "2022-01-10",
    salary: 78000,
    grossSalary: 7800,
    deductions: 936,
    netPay: 6864,
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "San Francisco, CA",
    manager: "Sarah Johnson",
    employmentType: "Full-time",
    gender: "Female",
    dob: "1993-04-15",
    address: "789 Market St, San Francisco, CA 94103",
    emergencyContact: "Kenji Tanaka — +1 (555) 567-8901",
    performance: 95,
  },
  {
    id: "EMP004",
    name: "James Carter",
    email: "james.carter@nexushr.com",
    phone: "+1 (555) 567-8901",
    department: "Finance",
    role: "Financial Analyst",
    designation: "Senior Financial Analyst",
    status: "On Leave",
    joinDate: "2019-09-05",
    salary: 88000,
    grossSalary: 8800,
    deductions: 1056,
    netPay: 7744,
    avatar: "https://images.unsplash.com/photo-1758876204244-930299843f07?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Boston, MA",
    manager: "Robert Chen",
    employmentType: "Full-time",
    gender: "Male",
    dob: "1985-08-30",
    address: "321 Commonwealth Ave, Boston, MA 02116",
    emergencyContact: "Amy Carter — +1 (555) 678-9012",
    performance: 85,
  },
  {
    id: "EMP005",
    name: "Emily Rodriguez",
    email: "emily.rodriguez@nexushr.com",
    phone: "+1 (555) 678-9012",
    department: "HR",
    role: "HR Specialist",
    designation: "HR Business Partner",
    status: "Active",
    joinDate: "2022-06-20",
    salary: 72000,
    grossSalary: 7200,
    deductions: 864,
    netPay: 6336,
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Austin, TX",
    manager: "Lisa Park",
    employmentType: "Full-time",
    gender: "Female",
    dob: "1995-02-14",
    address: "567 Congress Ave, Austin, TX 78701",
    emergencyContact: "Carlos Rodriguez — +1 (555) 789-0123",
    performance: 90,
  },
  {
    id: "EMP006",
    name: "Robert Chen",
    email: "robert.chen@nexushr.com",
    phone: "+1 (555) 789-0123",
    department: "Engineering",
    role: "VP Engineering",
    designation: "VP of Engineering",
    status: "Active",
    joinDate: "2018-02-01",
    salary: 145000,
    grossSalary: 14500,
    deductions: 1740,
    netPay: 12760,
    avatar: "https://images.unsplash.com/photo-1738566061505-556830f8b8f5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Seattle, WA",
    manager: "CEO",
    employmentType: "Full-time",
    gender: "Male",
    dob: "1980-07-19",
    address: "890 Pine St, Seattle, WA 98101",
    emergencyContact: "Wei Chen — +1 (555) 890-1234",
    performance: 97,
  },
  {
    id: "EMP007",
    name: "Priya Sharma",
    email: "priya.sharma@nexushr.com",
    phone: "+1 (555) 890-1234",
    department: "Product",
    role: "Product Manager",
    designation: "Senior Product Manager",
    status: "Active",
    joinDate: "2021-11-08",
    salary: 105000,
    grossSalary: 10500,
    deductions: 1260,
    netPay: 9240,
    avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "New York, NY",
    manager: "Robert Chen",
    employmentType: "Full-time",
    gender: "Female",
    dob: "1991-12-05",
    address: "234 5th Ave, New York, NY 10001",
    emergencyContact: "Raj Sharma — +1 (555) 901-2345",
    performance: 93,
  },
  {
    id: "EMP008",
    name: "Daniel Kim",
    email: "daniel.kim@nexushr.com",
    phone: "+1 (555) 901-2345",
    department: "Sales",
    role: "Sales Executive",
    designation: "Account Executive",
    status: "Inactive",
    joinDate: "2023-03-01",
    salary: 68000,
    grossSalary: 6800,
    deductions: 816,
    netPay: 5984,
    avatar: "https://images.unsplash.com/photo-1651684215020-f7a5b6610f23?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Los Angeles, CA",
    manager: "Marcus Williams",
    employmentType: "Part-time",
    gender: "Male",
    dob: "1997-09-22",
    address: "678 Sunset Blvd, Los Angeles, CA 90028",
    emergencyContact: "Jenny Kim — +1 (555) 012-3456",
    performance: 76,
  },
  {
    id: "EMP009",
    name: "Aisha Patel",
    email: "aisha.patel@nexushr.com",
    phone: "+1 (555) 112-2334",
    department: "Operations",
    role: "Operations Lead",
    designation: "Operations Manager",
    status: "Active",
    joinDate: "2020-04-12",
    salary: 92000,
    grossSalary: 9200,
    deductions: 1104,
    netPay: 8096,
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Dallas, TX",
    manager: "CEO",
    employmentType: "Full-time",
    gender: "Female",
    dob: "1989-03-08",
    address: "901 Commerce St, Dallas, TX 75201",
    emergencyContact: "Nikhil Patel — +1 (555) 223-3445",
    performance: 91,
  },
  {
    id: "EMP010",
    name: "Leo Martinez",
    email: "leo.martinez@nexushr.com",
    phone: "+1 (555) 334-4556",
    department: "Sales",
    role: "Sales Director",
    designation: "Director of Sales",
    status: "Active",
    joinDate: "2019-01-15",
    salary: 120000,
    grossSalary: 12000,
    deductions: 1440,
    netPay: 10560,
    avatar: "https://images.unsplash.com/photo-1758876204244-930299843f07?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    location: "Miami, FL",
    manager: "CEO",
    employmentType: "Full-time",
    gender: "Male",
    dob: "1983-05-27",
    address: "1234 Brickell Ave, Miami, FL 33131",
    emergencyContact: "Maria Martinez — +1 (555) 445-5667",
    performance: 94,
  },
];

export const departments = [
  { id: "D001", name: "Engineering", head: "Robert Chen", employees: 68, budget: 2800000, color: "#059669" },
  { id: "D002", name: "Marketing", head: "Marcus Williams", employees: 42, budget: 950000, color: "#14B8A6" },
  { id: "D003", name: "Design", head: "Yuki Tanaka", employees: 28, budget: 720000, color: "#10B981" },
  { id: "D004", name: "Finance", head: "James Carter", employees: 31, budget: 1100000, color: "#F59E0B" },
  { id: "D005", name: "HR", head: "Emily Rodriguez", employees: 23, budget: 600000, color: "#22C55E" },
  { id: "D006", name: "Product", head: "Priya Sharma", employees: 36, budget: 1500000, color: "#047857" },
  { id: "D007", name: "Sales", head: "Leo Martinez", employees: 55, budget: 1800000, color: "#EF4444" },
  { id: "D008", name: "Operations", head: "Aisha Patel", employees: 29, budget: 1300000, color: "#0EA5E9" },
];

export const departmentHeadcount = [
  { department: "Engineering", count: 68 },
  { department: "Sales", count: 55 },
  { department: "Marketing", count: 42 },
  { department: "Product", count: 36 },
  { department: "Finance", count: 31 },
  { department: "Operations", count: 29 },
  { department: "Design", count: 28 },
  { department: "HR", count: 23 },
];

export const attendanceOverview = [
  { name: "Present", value: 219, color: "#22C55E" },
  { name: "Absent", value: 12, color: "#EF4444" },
  { name: "On Leave", value: 17, color: "#F59E0B" },
];

export const recentActivities = [
  {
    id: 1,
    type: "join",
    text: "Alex Thompson joined as Senior Developer",
    subtext: "Engineering Department",
    time: "2 mins ago",
    icon: "UserPlus",
    color: "#22C55E",
  },
  {
    id: 2,
    type: "leave",
    text: "James Carter's leave was approved",
    subtext: "April 1 – April 7, 2026",
    time: "1 hour ago",
    icon: "Calendar",
    color: "#F59E0B",
  },
  {
    id: 3,
    type: "payroll",
    text: "March 2026 payroll processed",
    subtext: "₹412,800 total disbursed",
    time: "3 hours ago",
    icon: "DollarSign",
    color: "#059669",
  },
  {
    id: 4,
    type: "hire",
    text: "New job posted: UX Research Lead",
    subtext: "Design Department • Remote",
    time: "5 hours ago",
    icon: "Briefcase",
    color: "#14B8A6",
  },
  {
    id: 5,
    type: "review",
    text: "Q1 performance reviews completed",
    subtext: "248 employees evaluated",
    time: "1 day ago",
    icon: "Star",
    color: "#F59E0B",
  },
  {
    id: 6,
    type: "training",
    text: "Leadership training scheduled",
    subtext: "April 15 – April 16, 2026",
    time: "2 days ago",
    icon: "BookOpen",
    color: "#0EA5E9",
  },
];

// Attendance calendar for April 2026
export const attendanceCalendar: Record<number, "Present" | "Absent" | "Leave" | "Holiday" | "Weekend"> = {
  1: "Present",
  2: "Present",
  3: "Present",
  4: "Weekend",
  5: "Weekend",
  6: "Present",
  7: "Present",
  8: "Present",
  9: "Present",
  10: "Present",
  11: "Weekend",
  12: "Weekend",
  13: "Present",
  14: "Present",
  15: "Leave",
  16: "Leave",
  17: "Present",
  18: "Weekend",
  19: "Weekend",
  20: "Present",
  21: "Holiday",
  22: "Present",
  23: "Present",
  24: "Present",
  25: "Weekend",
  26: "Weekend",
  27: "Present",
  28: "Present",
  29: "Present",
  30: "Present",
};

export const dailyLogs = [
  { date: "Apr 01, 2026", checkIn: "08:58 AM", checkOut: "06:02 PM", hours: "9h 04m", status: "Present" },
  { date: "Apr 02, 2026", checkIn: "09:05 AM", checkOut: "06:15 PM", hours: "9h 10m", status: "Present" },
  { date: "Apr 03, 2026", checkIn: "08:47 AM", checkOut: "05:55 PM", hours: "9h 08m", status: "Present" },
  { date: "Apr 06, 2026", checkIn: "09:12 AM", checkOut: "06:30 PM", hours: "9h 18m", status: "Present" },
  { date: "Apr 07, 2026", checkIn: "08:55 AM", checkOut: "06:10 PM", hours: "9h 15m", status: "Present" },
  { date: "Apr 08, 2026", checkIn: "09:00 AM", checkOut: "06:05 PM", hours: "9h 05m", status: "Present" },
  { date: "Apr 09, 2026", checkIn: "08:50 AM", checkOut: "05:50 PM", hours: "9h 00m", status: "Present" },
  { date: "Apr 10, 2026", checkIn: "09:20 AM", checkOut: "06:25 PM", hours: "9h 05m", status: "Present" },
  { date: "Apr 13, 2026", checkIn: "09:02 AM", checkOut: "06:08 PM", hours: "9h 06m", status: "Present" },
  { date: "Apr 14, 2026", checkIn: "08:48 AM", checkOut: "05:52 PM", hours: "9h 04m", status: "Present" },
  { date: "Apr 15, 2026", checkIn: "—", checkOut: "—", hours: "—", status: "Leave" },
  { date: "Apr 16, 2026", checkIn: "—", checkOut: "—", hours: "—", status: "Leave" },
];

export const payrollEmployees = [
  {
    id: "EMP001",
    name: "Sarah Johnson",
    designation: "Senior Software Engineer",
    department: "Engineering",
    avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 9500,
    deductions: 1140,
    net: 8360,
    status: "Paid",
  },
  {
    id: "EMP002",
    name: "Marcus Williams",
    designation: "Marketing Manager",
    department: "Marketing",
    avatar: "https://images.unsplash.com/photo-1651684215020-f7a5b6610f23?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 8500,
    deductions: 1020,
    net: 7480,
    status: "Paid",
  },
  {
    id: "EMP003",
    name: "Yuki Tanaka",
    designation: "Lead UX Designer",
    department: "Design",
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 7800,
    deductions: 936,
    net: 6864,
    status: "Pending",
  },
  {
    id: "EMP004",
    name: "James Carter",
    designation: "Senior Financial Analyst",
    department: "Finance",
    avatar: "https://images.unsplash.com/photo-1758876204244-930299843f07?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 8800,
    deductions: 1056,
    net: 7744,
    status: "Paid",
  },
  {
    id: "EMP005",
    name: "Emily Rodriguez",
    designation: "HR Business Partner",
    department: "HR",
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 7200,
    deductions: 864,
    net: 6336,
    status: "Pending",
  },
  {
    id: "EMP006",
    name: "Robert Chen",
    designation: "VP of Engineering",
    department: "Engineering",
    avatar: "https://images.unsplash.com/photo-1738566061505-556830f8b8f5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 14500,
    deductions: 1740,
    net: 12760,
    status: "Paid",
  },
  {
    id: "EMP007",
    name: "Priya Sharma",
    designation: "Senior Product Manager",
    department: "Product",
    avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 10500,
    deductions: 1260,
    net: 9240,
    status: "Pending",
  },
  {
    id: "EMP008",
    name: "Leo Martinez",
    designation: "Director of Sales",
    department: "Sales",
    avatar: "https://images.unsplash.com/photo-1758876204244-930299843f07?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    gross: 12000,
    deductions: 1440,
    net: 10560,
    status: "Paid",
  },
];

export const recruitmentPipeline = {
  Applied: [
    {
      id: "C001",
      name: "Jordan Blake",
      role: "Senior React Developer",
      date: "Apr 1, 2026",
      avatar: "https://images.unsplash.com/photo-1651684215020-f7a5b6610f23?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "JB",
    },
    {
      id: "C002",
      name: "Mei Lin",
      role: "Data Analyst",
      date: "Apr 2, 2026",
      avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "ML",
    },
    {
      id: "C003",
      name: "Tyler Brooks",
      role: "Marketing Specialist",
      date: "Apr 3, 2026",
      avatar: null,
      initials: "TB",
    },
  ],
  Screening: [
    {
      id: "C004",
      name: "Sofia Garcia",
      role: "Product Designer",
      date: "Mar 28, 2026",
      avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "SG",
    },
    {
      id: "C005",
      name: "Noah Williams",
      role: "Sales Executive",
      date: "Mar 27, 2026",
      avatar: null,
      initials: "NW",
    },
  ],
  Interview: [
    {
      id: "C006",
      name: "Priya Nair",
      role: "HR Specialist",
      date: "Mar 22, 2026",
      avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "PN",
    },
    {
      id: "C007",
      name: "Carlos Mendez",
      role: "Backend Engineer",
      date: "Mar 20, 2026",
      avatar: "https://images.unsplash.com/photo-1758876204244-930299843f07?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "CM",
    },
    {
      id: "C008",
      name: "Anika Patel",
      role: "Financial Analyst",
      date: "Mar 19, 2026",
      avatar: null,
      initials: "AP",
    },
  ],
  "Offer Sent": [
    {
      id: "C009",
      name: "Ethan Moore",
      role: "DevOps Engineer",
      date: "Mar 15, 2026",
      avatar: "https://images.unsplash.com/photo-1738566061505-556830f8b8f5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "EM",
    },
    {
      id: "C010",
      name: "Lena Fischer",
      role: "UX Researcher",
      date: "Mar 14, 2026",
      avatar: null,
      initials: "LF",
    },
  ],
  Hired: [
    {
      id: "C011",
      name: "Alex Thompson",
      role: "Senior React Developer",
      date: "Mar 10, 2026",
      avatar: "https://images.unsplash.com/photo-1651684215020-f7a5b6610f23?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "AT",
    },
    {
      id: "C012",
      name: "Hana Yamamoto",
      role: "Product Designer",
      date: "Mar 8, 2026",
      avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
      initials: "HY",
    },
  ],
};

export const performanceData = [
  { name: "Q1 2025", score: 78 },
  { name: "Q2 2025", score: 82 },
  { name: "Q3 2025", score: 85 },
  { name: "Q4 2025", score: 88 },
  { name: "Q1 2026", score: 92 },
];

export const leaveRequests = [
  {
    id: "LR001",
    employee: "Sarah Johnson",
    avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    type: "Annual Leave",
    from: "Apr 10",
    to: "Apr 14",
    days: 5,
    status: "Pending",
  },
  {
    id: "LR002",
    employee: "Marcus Williams",
    avatar: "https://images.unsplash.com/photo-1651684215020-f7a5b6610f23?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    type: "Sick Leave",
    from: "Apr 7",
    to: "Apr 8",
    days: 2,
    status: "Approved",
  },
  {
    id: "LR003",
    employee: "Yuki Tanaka",
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    type: "Personal Leave",
    from: "Apr 20",
    to: "Apr 21",
    days: 2,
    status: "Pending",
  },
  {
    id: "LR004",
    employee: "James Carter",
    avatar: "https://images.unsplash.com/photo-1758876204244-930299843f07?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    type: "Annual Leave",
    from: "Apr 1",
    to: "Apr 7",
    days: 5,
    status: "Approved",
  },
  {
    id: "LR005",
    employee: "Emily Rodriguez",
    avatar: "https://images.unsplash.com/photo-1762522921456-cdfe882d36c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    type: "Sick Leave",
    from: "Mar 28",
    to: "Mar 29",
    days: 2,
    status: "Rejected",
  },
  {
    id: "LR006",
    employee: "Priya Sharma",
    avatar: "https://images.unsplash.com/photo-1765005204058-10418f5123c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=200",
    type: "Annual Leave",
    from: "Apr 15",
    to: "Apr 16",
    days: 2,
    status: "Pending",
  },
];

export const trainingCourses = [
  {
    id: "TC001",
    title: "React & TypeScript Mastery",
    category: "Engineering",
    instructor: "Sarah Johnson",
    duration: "12h 30m",
    progress: 75,
    status: "In Progress",
    enrolled: 24,
    level: "Intermediate",
    accentColor: "#10B981",
    bgColor: "#ECFDF5",
    description: "Deep dive into React patterns, hooks, and TypeScript best practices.",
  },
  {
    id: "TC002",
    title: "Leadership & Team Management",
    category: "Management",
    instructor: "Robert Chen",
    duration: "8h 00m",
    progress: 100,
    status: "Completed",
    enrolled: 18,
    level: "Advanced",
    accentColor: "#6366F1",
    bgColor: "#EEF2FF",
    description: "Develop leadership skills for managing high-performance teams.",
  },
  {
    id: "TC003",
    title: "HR Compliance & Labor Law",
    category: "HR",
    instructor: "Lisa Park",
    duration: "6h 15m",
    progress: 40,
    status: "In Progress",
    enrolled: 31,
    level: "Beginner",
    accentColor: "#F59E0B",
    bgColor: "#FFFBEB",
    description: "Understanding employment law, compliance, and HR best practices.",
  },
  {
    id: "TC004",
    title: "Data Analytics with Power BI",
    category: "Analytics",
    instructor: "Marcus Williams",
    duration: "10h 45m",
    progress: 0,
    status: "Not Started",
    enrolled: 15,
    level: "Intermediate",
    accentColor: "#3B82F6",
    bgColor: "#EFF6FF",
    description: "Master Power BI dashboards, DAX formulas and data modelling.",
  },
  {
    id: "TC005",
    title: "Payroll Management Systems",
    category: "Finance",
    instructor: "James Carter",
    duration: "5h 00m",
    progress: 100,
    status: "Completed",
    enrolled: 22,
    level: "Beginner",
    accentColor: "#059669",
    bgColor: "#ECFDF5",
    description: "Complete guide to payroll processing, tax calculations, and compliance.",
  },
  {
    id: "TC006",
    title: "Cybersecurity Awareness",
    category: "IT",
    instructor: "Yuki Tanaka",
    duration: "4h 30m",
    progress: 60,
    status: "In Progress",
    enrolled: 48,
    level: "Beginner",
    accentColor: "#EF4444",
    bgColor: "#FEF2F2",
    description: "Essential security practices, phishing prevention, and data protection.",
  },
];

export const certifications = [
  { id: "CERT001", title: "Certified HR Professional", employee: "Ryan Park", issueDate: "Mar 2026", expiry: "Mar 2028", color: "#10B981" },
  { id: "CERT002", title: "AWS Cloud Practitioner", employee: "Sarah Johnson", issueDate: "Jan 2026", expiry: "Jan 2029", color: "#F59E0B" },
  { id: "CERT003", title: "PMP Certification", employee: "Robert Chen", issueDate: "Nov 2025", expiry: "Nov 2028", color: "#6366F1" },
  { id: "CERT004", title: "Google Analytics", employee: "Marcus Williams", issueDate: "Feb 2026", expiry: "Feb 2027", color: "#3B82F6" },
];

export const documents = [
  {
    id: "DOC001",
    name: "Employment Contract — Sarah Johnson.pdf",
    category: "Contracts",
    date: "Jan 15, 2026",
    size: "245 KB",
    type: "pdf",
    uploadedBy: "Ryan Park",
  },
  {
    id: "DOC002",
    name: "Payslip March 2026 — Marcus Williams.pdf",
    category: "Payslips",
    date: "Mar 31, 2026",
    size: "128 KB",
    type: "pdf",
    uploadedBy: "System",
  },
  {
    id: "DOC003",
    name: "Remote Work Policy 2026.docx",
    category: "Policies",
    date: "Feb 10, 2026",
    size: "89 KB",
    type: "docx",
    uploadedBy: "Lisa Park",
  },
  {
    id: "DOC004",
    name: "Annual Leave Summary Q1 2026.xlsx",
    category: "Reports",
    date: "Apr 1, 2026",
    size: "312 KB",
    type: "xlsx",
    uploadedBy: "Ryan Park",
  },
  {
    id: "DOC005",
    name: "Offer Letter — Priya Sharma.pdf",
    category: "Contracts",
    date: "Mar 20, 2026",
    size: "198 KB",
    type: "pdf",
    uploadedBy: "Robert Chen",
  },
  {
    id: "DOC006",
    name: "Code of Conduct 2026.pdf",
    category: "Policies",
    date: "Jan 2, 2026",
    size: "432 KB",
    type: "pdf",
    uploadedBy: "Lisa Park",
  },
  {
    id: "DOC007",
    name: "Payslip March 2026 — Sarah Johnson.pdf",
    category: "Payslips",
    date: "Mar 31, 2026",
    size: "131 KB",
    type: "pdf",
    uploadedBy: "System",
  },
  {
    id: "DOC008",
    name: "Q1 2026 Headcount Report.xlsx",
    category: "Reports",
    date: "Apr 5, 2026",
    size: "520 KB",
    type: "xlsx",
    uploadedBy: "Ryan Park",
  },
];

export const faqItems = [
  {
    id: "FAQ001",
    question: "How do I add a new employee?",
    answer: "Navigate to the Employees page, click the '+ Add Employee' button in the top right, fill in the employee details form, and click Save. The employee will be added immediately.",
    category: "Employees",
  },
  {
    id: "FAQ002",
    question: "How do I approve a leave request?",
    answer: "Go to Leave Management, find the pending request in the list, and click the green checkmark to approve or the red X to reject. The employee will be notified automatically.",
    category: "Leave",
  },
  {
    id: "FAQ003",
    question: "How is the net salary calculated?",
    answer: "Net salary = Gross Salary − Deductions. Deductions include tax, provident fund (12% of basic), and any other statutory deductions configured in Settings.",
    category: "Payroll",
  },
  {
    id: "FAQ004",
    question: "How do I run a payroll for a specific month?",
    answer: "Open the Payroll page, select the month from the date picker, review all entries, then click 'Process Payroll'. A confirmation dialog will appear before finalizing.",
    category: "Payroll",
  },
  {
    id: "FAQ005",
    question: "How do I export attendance reports?",
    answer: "Go to Reports, select the 'Attendance' report type, choose your date range, and click 'Export as CSV' or 'Export as PDF'. Reports can be scheduled for automatic delivery too.",
    category: "Reports",
  },
  {
    id: "FAQ006",
    question: "Can I customize department structure?",
    answer: "Yes! Go to Departments, click on any department to edit it, or use the '+ Add Department' button. You can set a department head, budget, and team members.",
    category: "Departments",
  },
  {
    id: "FAQ007",
    question: "How do I change my password?",
    answer: "Click your avatar in the top right, select 'Settings', then navigate to the 'Security' tab. Enter your current password and your new password twice to confirm.",
    category: "Settings",
  },
  {
    id: "FAQ008",
    question: "What does the Smart Search do?",
    answer: "Smart Search lets you instantly find employees, departments, documents, and records across the entire platform. Just press Ctrl+K or click the Search icon in the sidebar.",
    category: "General",
  },
];

export const helpCategories = [
  { id: "HC001", title: "Getting Started", description: "Setup, onboarding, and first steps", icon: "Rocket", count: 12, color: "#10B981", bg: "#ECFDF5" },
  { id: "HC002", title: "Employee Management", description: "Add, edit, and manage employees", icon: "Users", count: 18, color: "#6366F1", bg: "#EEF2FF" },
  { id: "HC003", title: "Payroll & Finance", description: "Salary, deductions, and processing", icon: "IndianRupee", count: 14, color: "#F59E0B", bg: "#FFFBEB" },
  { id: "HC004", title: "Attendance & Leave", description: "Time tracking and leave requests", icon: "Calendar", count: 10, color: "#3B82F6", bg: "#EFF6FF" },
  { id: "HC005", title: "Reports & Analytics", description: "Export, schedule, and analyze data", icon: "BarChart3", count: 9, color: "#EF4444", bg: "#FEF2F2" },
  { id: "HC006", title: "Settings & Security", description: "Account, roles, and permissions", icon: "Settings", count: 11, color: "#8B5CF6", bg: "#F5F3FF" },
];
